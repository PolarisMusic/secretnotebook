...
describe('ledger — awardCouplePoints (R7 revived writer)', () => {
  it('inserts a couple_points row and enqueues a ledger_entry.add op', async () => {
    ...
    await awardCouplePoints(deps, { delta: 500, reason: 'unlock_verified', refId: REF });

    expect(await sumConnectionPoints(deps.exec)).toBe(500);
  ...
  it('treats verify and reflection awards on the same attempt as distinct (different reason)', async () => {
    ...
    await awardCouplePoints(deps, { delta: 500, reason: 'unlock_verified', refId: REF });
    await awardCouplePoints(deps, { delta: 500, reason: 'unlock_reflected', refId: REF });
    expect(await sumConnectionPoints(deps.exec)).toBe(1000);
  });
