...
## 1. Native patches applied automatically during `expo prebuild`

The Expo bare workflow regenerates the native projects on every prebuild.
Anything that has to survive that regeneration lives as a local config
plugin under `apps/mobile/plugins/`, registered in `app.json`'s
`expo.plugins` array. Prebuild runs them as part of its mod chain so
operators don't re-apply edits by hand.

Two plugins ship today; both are idempotent (re-running prebuild against
an already-patched file is a no-op).

### 1.1 `FLAG_SECURE` on Android — `with-android-flag-secure.js`
...
surface gets it. The simplest implementation is setting the flag on the
host `Activity`, which covers every screen rendered into it.

The plugin uses `withMainActivity` from `@expo/config-plugins` to
inject the import + `window.setFlags(...)` call into
`MainActivity.kt` during prebuild. The JS-side `useSecureScreen()`
hook in `src/security/secure-screen.ts` stays as a forward-looking
stub for per-screen control — the activity-level patch is what
actually sets the flag today.
...
### 1.2 Release-signing config — `with-android-release-signing.js`

Prebuild only emits a `debug` signingConfig and wires
`buildTypes.release.signingConfig` to it (so
`expo run:android --variant release` works locally without a
keystore). For the Play upload we need a real `release` signingConfig
pointing at the operator's keystore.

The plugin uses `withAppBuildGradle` to:

- inject a `release { storeFile ... }` block alongside `debug`
- swap `buildTypes.release.signingConfig` from `signingConfigs.debug`
  to `signingConfigs.release`

The keystore path + passwords are read at build time via
`findProperty('MYAPP_RELEASE_*')`, falling back to the debug keystore
so `expo run:android --variant release` keeps working on a machine
that has no release keystore configured. For the Play upload, set
the real values in `android/gradle.properties` (or via
`-PMYAPP_RELEASE_*` on the gradlew CLI) before running
`./gradlew bundleRelease` — see §5.2 for the canonical sequence.

### 1.3 iOS — no patch needed in Phase 1
...
2. `pnpm --filter @secretnotebook/mobile expo:prebuild` (config plugins re-apply native patches automatically — see §1)
